...
Below is a plot of the results of the clustering algorithm. Consider that a user's song is located within cluster group 1, highlighted in red. The cKDTree method from the SciPy package will calculate the closest datapoints to the track's features within cluster group 1. The track_id's were then saved, where the track name, artist, and album cover can then be retrieved from the Spotify API. 
<p align="center">
